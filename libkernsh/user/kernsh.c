  ...
  if (!strcmp(os, "Linux"))
    {
      ...
      snprintf(buffer, sizeof(buffer), "%s-%s", LIBKERNSH_DEFAULT_LINUX_KERNEL,
               release);
 ...
 */
void  *kernsh_get_raw_by_addr(elfshobj_t *null, eresi_Addr addr, void *buf,
                              u_int size)
{
  void  *dataptr;
...
int kernsh_info_linux()
{
  ...
  elfsh_readmema(libkernshworld.root,
                 idtr.base + (2 * sizeof(unsigned long)) * 0x80,
                 &idt,
                 sizeof(idt));
...
int kernsh_raw_write(elfshobj_t *file, u_int foffset, void *src_buff, int len)
{
  ...
  if (elfsh_is_runtime_mode())
    {
      elfsh_writemem(file, elfsh_get_vaddr_from_foffset(file, foffset), src_buff,
                     len);
...
int kernsh_raw_read(elfshobj_t *file,  u_int foffset, void *dest_buff, int len)
{
  ...
  if (elfsh_is_runtime_mode())
    {
      elfsh_readmema(file, elfsh_get_vaddr_from_foffset(file, foffset), dest_buff,
                     len);
      PROFILER_ROUT(__FILE__, __FUNCTION__, __LINE__, len);
    }
