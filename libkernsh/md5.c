 ...
 * make it compile on machines like Cray that don't have a 32 bit 
 * integer type. 
 *
 *
 ...
 */
int  kernsh_md5init(libkernshmd5context_t *ctx)
{
 ...
 * of bytes.
 */
int kernsh_md5update(libkernshmd5context_t *ctx, 
		     unsigned char *buf, 
		     unsigned len)
{
  unsigned long t;
  ...
  if (t) 
      ...
      if (len < t) 
	{
      ...
      memcpy(p, buf, t);
      kernsh_md5transform(ctx->buf, ctx->in);
      buf += t;
      len -= t;
    }
 ...
 */
int kernsh_md5final(unsigned char digest[16], libkernshmd5context_t *ctx)
{
  unsigned count;
		    ...
		    unsigned char md5buffer[BUFSIZ])
{
  ...
  kernsh_md5init(&md);
  kernsh_md5update(&md, pass, value);
  kernsh_md5final(md5buffer, &md);
