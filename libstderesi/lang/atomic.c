...
int     cmd_set()
{
  ...
  if (hash_get(&world.curjob->recur[world.curjob->curscope].exprs,
  ...
  if (e2 && e2->value && e2->type->type == ASPECT_TYPE_OID)
    {
      oid = (e2->value->immed ? e2->value->immed_val.word : e2->value->get_obj(
               e2->value->parent));
  ...
  if (!e1)
    {
      if (world.curjob->curcmd->param[0][0] == REVM_VAR_PREFIX)
        {
          e1 = revm_expr_extend(world.curjob->curcmd->param[0],
