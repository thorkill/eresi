...
#if __DEBUG_FOREACH__
  fprintf(stderr,
          " [D] INDUCTION_LOAD: curcmd->end = %s and iter[cur].end = %s \n",
...
static revmexpr_t *revm_induction_get(char *name)
{
  ...
  /* If we are reusing the same name for the induction variable */
  else if (!strcmp(name, world.curjob->iter[world.curjob->curloop].curind->label))
    {
      ...
      if (NULL != world.curjob->iter[world.curjob->curloop].end &&
          ...
          strcmp(world.curjob->iter[world.curjob->curloop].end,
                 world.curjob->curcmd->endlabel))
        {
          PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
      ...
      if (world.curjob->iter[world.curjob->curloop].reclevel !=
  ...
  else if (world.curjob->iter[world.curjob->curloop].reclevel !=
           world.curjob->curscope ||
...
#if __DEBUG_FOREACH__
  fprintf(stderr,
          "INDUCTION_GET: Returning with Induction variable name = %s elmidx = %u \n",
          name,
          world.curjob->iter[world.curjob->curloop].elmidx);
#endif
      ...
      !regexec(world.curjob->iter[world.curjob->curloop].usedregx, curkey, 0, 0, 0))
    {
      ...
      /* Case of a complex induction variable (expr with children) */
      else
        {
          ...
          if (table)
            {
              ...
              name = table->name;
              transnbr = table->elmnbr;
            }
          else
            {
              elist_set(list, curkey, induction);
              name = list->name;
              transnbr = list->elmnbr;
            }
          ...
          ** - in the previous induction variable (the rewritten expression of outter loop)
          ** - in the previous container (the one iterated on in outter loop)
          */
          if (!strcmp(name, "transformed") && world.curjob->curloop > 0)
            {
              if (world.curjob->iter[world.curjob->curloop - 1].tcontainer ==
                  REVM_CONTAINER_LIST)
                {
              ...
              else if (world.curjob->iter[world.curjob->curloop - 1].tcontainer ==
                       REVM_CONTAINER_HASH)
                {
                               ...
                               char *tableorlist)
{
  ...
  /* Destroy previous iteration of the induction variable */
...
#if __DEBUG_FOREACH__
  fprintf(stderr,
          "ITERATOR_FREE: HIDE induction %s and decrementing curloop ! \n", indname);
#endif
...

{
  ...
  if (aspect_type_simple(induction->type->type) && !induction->type->next)
  ...
  else
    {
      ...
      induction = revm_inform_type_addr(typename, paramname, (eresi_Addr) elem, NULL,
                                        0, 1);
...
int   cmd_foreach()
{
  ...
  switch (looptype)
    ...
    case REVM_LOOP_REGEX:
      world.curjob->iter[world.curjob->curloop].usedregx =
        &world.curjob->curcmd->regx[0];
      container = revm_iterator_get(world.curjob->curcmd->param[2], &keys, &keynbr,
                                    &tableorlist);
...
nextelem:
      ...
      if (looptype == REVM_LOOP_REGEX
          && regexec(&world.curjob->curcmd->regx[0], keys[index], 0, 0, 0))
        {
          goto nextelem;
        }
    ...
    default:
      PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__, "Invalid foreach parameters",
                   -1);
    }
...
int   cmd_forend()
{
  ...
  else if (world.curjob->iter[world.curjob->curloop].tcontainer ==
           REVM_CONTAINER_HASH)
    {
  ...
  if (world.curjob->iter[world.curjob->curloop].curind != NULL)
    {
      ...
      if (ret < 0)
        {
          PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__, "Failed terminate iteration",
                       -1);
        }
    }
