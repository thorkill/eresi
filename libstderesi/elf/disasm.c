 ...
 * @return
 */
int             revm_object_display(elfshsect_t *parent, elfsh_Sym *sym,
                                    int size,
                                    ...
                                    eresi_Addr vaddr, char *name, char otype, u_char addbase)
{
  ...
  switch (otype)
      ...
      do
        {
          ...
          for (startaddr = vaddr, readsz = nbrinstr = 0;
               ...
               nbrinstr++)
            {
              value = revm_instr_display(-1, vaddr, foffset, readnow, name, idx_bytes,
                                         buff + readsz);
    ...
    case REVM_VIEW_HEX:
      ...
      do
        {
          value = revm_hexa_display(parent, name, vaddr, readnow, symoff + index, buff,
                                    foffset);
  ...
  if (parent && elfsh_is_runtime_mode() && (kernsh_is_present()
      || kedbg_is_present()))
    {
