 ...
 */
static int  revm_field_propagate(revmexpr_t *dest, revmexpr_t *source,
                                 char *fname)
{
  char    srcname[BUFSIZ];
  char    dstname[BUFSIZ];
  ...
  if (!dst)
    {
      ...
      memcpy((char *) dstannot->addr + dest->type->size, (char *) addedannot->addr,
             child->type->size);
...
static int  revm_case_transform(revmexpr_t *matchme, char *destvalue)
{
  ...
  for (curidx = world.curjob->iter[world.curjob->curloop].elmidx - 1,
       curptr = destvalue;
       curptr && *curptr;
       curptr = foundptr + 2, curidx++, dstnbr++)
    {
      ...
      snprintf(namebuf, BUFSIZ, "%s-%u",
               world.curjob->iter[world.curjob->curloop].curkey, curidx);
  ...
  else if (dstnbr == 1)
    {
      ...
      if (!strcmp(destvalue, "."))
      ...
      else
        {
          ...
          ret = elist_set(looplist, world.curjob->iter[world.curjob->curloop].curkey,
                          candid);
          ...
          */
          elist_append(exprlist, strdup(world.curjob->iter[world.curjob->curloop].curkey),
                       candid);
...
static int  revm_case_execmd(char *str)
{
  ...
  snprintf(actual, sizeof(actual), "job%u_rec%u_labels", world.curjob->id,
           world.curjob->curscope);
  hash_init(&world.curjob->recur[world.curjob->curscope].labels,
            strdup(actual), 11, ASPECT_TYPE_STR);
  snprintf(actual, sizeof(actual), "job%u_rec%u_exprs", world.curjob->id,
           world.curjob->curscope);
  hash_init(&world.curjob->recur[world.curjob->curscope].exprs, strdup(actual), 1,
            ASPECT_TYPE_EXPR);
...
int   cmd_into()
{
  ...
  if (revm_case_transform(
        world.curjob->recur[world.curjob->curscope].rwrt.matchexpr,
...
int   cmd_case()
{
  ...
  candid = revm_expr_create(matchme->type, "$candid",
                            strdup(world.curjob->curcmd->param[0]));
...
int   cmd_default()
{
  ...
  hash_init(&world.curjob->recur[world.curjob->curscope].labels, strdup(actual),
            3, ASPECT_TYPE_UNKNOW);
  snprintf(actual, sizeof(actual), "job%u_rec%u_exprs",
           world.curjob->id, world.curjob->curscope);
  hash_init(&world.curjob->recur[world.curjob->curscope].exprs, strdup(actual), 7,
            ASPECT_TYPE_UNKNOW);
