  ...
  for (buffer = 0, listent = linklist->head; listent; listent = listent->next)
    {
      ...
      switch (caller->type)
        ...
        case MJR_LINK_FUNC_CALL:
          ...
          asm_read_instr(&ins, (u_char *) buffer + cal->size - len, len,
                         world.curjob->proc);
          break;
        ...
        case MJR_LINK_BLOCK_COND_ALWAYS:
          len = asm_read_instr(&ins, (u_char *) buffer + cal->size - 2, 2,
                               world.curjob->proc);
          ...
          len = asm_read_instr(&ins, (u_char *) buffer + cal->size - 3, 3,
                               world.curjob->proc);
          ...
          len = asm_read_instr(&ins, (u_char *) buffer + cal->size - 4, 4,
                               world.curjob->proc);
          ...
          len = asm_read_instr(&ins, (u_char *) buffer + cal->size - 5, 5,
                               world.curjob->proc);
