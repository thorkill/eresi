...
int   revm_graph_get_function_type(mjrfunc_t *fnc)
{
  ...
  if (fnc->vaddr == elfsh_get_entrypoint(elfsh_get_hdr(
      world.mjr_session.cur->obj)))
    {
  ...
  for (curent = linklist->head; curent; curent = curent->next)
    {
      ...
      cblk = (mjrblock_t *) mjr_lookup_container(world.mjr_session.cur,
             lnk->id)->data;
      ...
      sname = elfsh_reverse_metasym(world.mjr_session.cur->obj, cblk->vaddr,
                                    &soffset);
  ...
  for (curent = linklist->head; curent; curent = curent->next)
    {
      ...
      tmpfnc = (mjrfunc_t *) mjr_lookup_container(world.mjr_session.cur,
               curlnk->id)->data;
      ...
      if (!mjr_analysed(&world.mjr_session, world.curjob->curfile->hdr->e_entry))
        {
          ...
          ret = mjr_analyse(&world.mjr_session, world.curjob->curfile->hdr->e_entry,
                            maxdepth, 0);
