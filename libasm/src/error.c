  ...
  if (proc)
    {
      switch (proc->error_code)
        {
        case LIBASM_ERROR_SUCCESS:
          return (LIBASM_MSG_SUCCESS);

        case LIBASM_ERROR_NSUCH_CONTENT:
          return (LIBASM_MSG_SUCCESS);

        case LIBASM_ERROR_ILLEGAL:
          return (LIBASM_MSG_SUCCESS);

        case LIBASM_ERROR_TOOSHORT:
          return (LIBASM_MSG_TOOSHORT);
        }
