  ...
  if (stabs_info->index_list >= stabs_info->num_list)
    {
      ...
      if (stabs_info->dir_list == NULL)
        {
          XALLOC(__FILE__, __FUNCTION__, __LINE__, stabs_info->dir_list,
                 stabs_info->num_list * sizeof(char *), -1);
          XALLOC(__FILE__, __FUNCTION__, __LINE__, stabs_info->file_list,
                 stabs_info->num_list * sizeof(char *), -1);
        }
      else
        {
          XREALLOC(__FILE__, __FUNCTION__, __LINE__, stabs_info->dir_list,
                   stabs_info->dir_list,
                   stabs_info->num_list * sizeof(char *), -1);
          XREALLOC(__FILE__, __FUNCTION__, __LINE__, stabs_info->file_list,
                   stabs_info->file_list,
                   stabs_info->num_list * sizeof(char *), -1);
        }
    }
  ...
  else
    {
      ...
      switch (token)
        ...
        case STABS_STR_T_GFUNC:
          data->scope = token == STABS_STR_T_FFUNC ? STABS_SCOPE_LFUNC :
                        STABS_SCOPE_GFUNC;
...
int     edfmt_stabs_parse(elfshobj_t *file)
{
  ...
  XALLOC(__FILE__, __FUNCTION__, __LINE__, stabs_info, sizeof(edfmtstabsinfo_t),
         -1);
