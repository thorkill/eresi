...
static revmexpr_t *revm_expr_read(char **datavalue)
{
  ...
  if (opening == closing)
    {
      for (*datavalue = datastr--; *datastr == ')' && *namend == '('
           && beginning-- > 0;
           *datastr-- = 0x00)
        {
          *namend++ = 0x00;
                                   ...
                                   void *srcdata)
{
  ...
  newexpr->value = revm_object_lookup_real(parenttype, recpath,
...

{
  ...
  if (newexpr->strval[0] == REVM_VAR_PREFIX)
    {
      ...
      len = snprintf(recpath + ctx->pathsize, BUFSIZ - ctx->pathsize, ".%s",
                     newexpr->label);
...
#if __DEBUG_EXPRS__
      fprintf(stderr,
              " [D] COPIED EXISTING INITIAL REVMEXPR = %s :: %s (new name) \n",
  ...
  len = snprintf(recpath + ctx->pathsize, BUFSIZ - ctx->pathsize, ".%s",
                 childtype->fieldname);
  revm_inform_type_addr(childtype->name, strdup(recpath), (eresi_Addr) childata,
                        newexpr, 0, 0);
...
#if __DEBUG_EXPRS_MORE__
      fprintf(stderr,
              " [D] Top level type with explicit type label = (%s) strval = (%s) \n",
              newexpr->label, newexpr->strval);
#endif
...
#if __DEBUG_EXPRS_MORE__
      fprintf(stderr,
              " [D] No explicit top-level type, continuing with datavalue = %s \n",
              *datavalue);
#endif
...
#if __DEBUG_EXPRS__
  fprintf(stderr,
          " [D] Entering revm_expr_init with toplevel = %u , curpath = %s\n",
      ...
      if (source->childs)
        {
          ...
          revm_inform_type_addr(type->name, strdup(newname), (eresi_Addr) childata, dest,
                                0, 0);
...
revmexpr_t  *revm_expr_copy(revmexpr_t *source, char *dstname, u_char isfield)
{
...
#if __DEBUG_EXPRS__
  fprintf(stderr,
          " [D] Allocating copied expression data of size %u type %s to name %s (%s)\n",
  ...
  if (!revm_inform_type_addr(type->name, strdup(dstname), (eresi_Addr) copydata,
                             dest, 0, 0))
    PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
                 "Unable to inform copy expression", NULL);
  ...
  /* Some (structure) expressions have an extra top-level type, copy was done before the recursion */
    ...
    {
      XALLOC(__FILE__, __FUNCTION__, __LINE__, dest->childs, sizeof(revmexpr_t),
             NULL);
    ...
    u_char force)
{
  ...
  if (!copyme->otype || (aspect_type_simple(copyme->otype->type)
                         && !copyme->otype->next))
    {
      dest->value  = copyme;
    }
  else
    {
      ...
      if (!revm_inform_type_addr(type->name, strdup(name), (eresi_Addr) data, dest, 0,
                                 1))
        PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
...
int   revm_expr_print(revmexpr_t *expr, u_char quiet)
{
  ...
  ret = revm_expr_printrec(expr, (!iter
                                  || !expr->next ? strlen(name) + 6 : 1), 0, iter, quiet);
                                ...
                                eresi_Addr *val)
{
  ...
  if (!candidate || !original || *candidate != REVM_VAR_PREFIX
      || *original != REVM_VAR_PREFIX)
    PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
                 "Invalid name for expression", -1);
                              ...
                              char    *datavalue)
{
  ...
  realname = dataname;
  revm_inform_type_addr(datatype->name, strdup(realname), (eresi_Addr) databuff,
                        NULL, 0, 0);
                                     ...
                                     char *value)
{
  ...
  if (*name != REVM_VAR_PREFIX || !strncmp(name, "$hash", 5)
      || !strncmp(name, "$list", 5))
    {
      name = revm_tmpvar_create();
    }
...
aspectype_t *revm_exprtype_get(char *exprvalue)
{
  ...
  for (curexprvalue = exprvalue, typenamelen = 0; *curexprvalue != '(';
       typenamelen++)
    {
      curexprvalue++;
    }
  ...
  for (next = ctx->curexpr; next; next = supernext)
    {
      ...
      fprintf(stderr, "New loop iter on ->next with pathbuf = %s (will extend) \n",
              ctx->pathbuf);
#endif
      len = snprintf(ctx->pathbuf + ctx->pathsize,
                     sizeof(ctx->pathbuf) - ctx->pathsize,
                     ".%s", next->label);
      ctx->pathsize += len;
#if __DEBUG_EXPRS__
...
#if __DEBUG_EXPRS__
          fprintf(stderr,
                  " [D] UNLINK RECURSION FOR %s (exprfree = %u, datafree = %u) \n",
                  ctx->pathbuf, exprfree, datafree);
#endif
          backupexpr = ctx->curexpr;
