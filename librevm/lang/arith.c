...
static revmexpr_t *revm_deref(revmexpr_t *ref)
{
  ...
  switch (ref->type->type)
    ...
    case ASPECT_TYPE_DADDR:
      addr = (ref->value->immed ? ref->value->immed_val.ent : ref->value->get_obj(
                ref->value->parent));
      break;
  ...
  paddr = (eresi_Addr *) elfsh_readmema(world.curjob->curfile, addr, buff,
                                        sizeof(eresi_Addr));
 ...
 * @ingroup lang
 */
static revmexpr_t *revm_compute_intermediate(revmexpr_t **left, revmexpr_t *res,
    u_char *op)
{
  u_char    quiet;
  int     ret;
