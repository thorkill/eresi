...
int   revm_source(char **params)
{
  ...
  snprintf(actual, sizeof(actual), "job%u_rec%u_exprs", world.curjob->id,
           world.curjob->curscope);
  hash_init(&world.curjob->recur[world.curjob->curscope].exprs, strdup(actual),
            23, ASPECT_TYPE_EXPR);
                                ...
                                char  *savedname)
{
  ...
  keys = hash_get_keys(&world.curjob->recur[world.curjob->curscope].exprs,
                       &keynbr);
      ...
      if (!strcmp(keys[idx], REVM_VAR_RESULT) && world.curjob->curscope)
        {
          ...
          oldres = hash_get(&world.curjob->recur[world.curjob->curscope - 1].exprs,
                            REVM_VAR_RESULT);
          ...
          hash_add(&world.curjob->recur[world.curjob->curscope - 1].exprs,
                   strdup(keys[idx]), res);
