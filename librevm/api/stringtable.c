  ...
  if (strtable == NULL)
    {
      ...
      XALLOC(__FILE__, __FUNCTION__, __LINE__, strtable, strtable_max * sizeof(char),
             -1);
    }
  else if (strtable_max <= strtable_current + string_len)
    {
      ...
      XREALLOC(__FILE__, __FUNCTION__, __LINE__, strtable, strtable,
               strtable_max * sizeof(char), -1);
    }
