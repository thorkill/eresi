...
pkt_t *dump_recv_pkt(int s)
{
  ...
  if (ntohl(msg->size) != 0)
    {
      XALLOC(__FILE__, __FUNCTION__, __LINE__, msg->data,
             (size_t) sizeof (char)*ntohl(msg->size), (pkt_t *) -1);
...
int   dump_receive_RR(pkt_t *pkt)
{
  ...
  XALLOC(__FILE__, __FUNCTION__, __LINE__, npath,
         sizeof (dump_id_t) * (ntohl(pkt->path_len) + 1), -1);
