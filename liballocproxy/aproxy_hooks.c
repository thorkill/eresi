...
#endif
{
  ...
  victim = (top_check() >= 0) ? aproxy_int_malloc(&main_arena, sz + 1) : NULL;
...
#endif
  aproxy_int_free(&main_arena, mem);
  (void)mutex_unlock(&main_arena.mutex);
}
...
#endif
{
  ...
  if (chunk_is_mmapped(oldp))
...
#endif
        {
          ...
          if (oldsize - SIZE_SZ >= nb)
          ...
          else
            {
              ...
              if (top_check() >= 0)
                {
                  newmem = aproxy_int_malloc(&main_arena, bytes + 1);
                }
  ...
  else
    {
      ...
      if (top_check() >= 0)
        {
          newmem = aproxy_int_realloc(&main_arena, oldmem, bytes + 1);
        }
...
#endif
{
  ...
  checked_request2size(bytes + 1, nb);
  (void)mutex_lock(&main_arena.mutex);
  mem = (top_check() >= 0) ? aproxy_int_memalign(&main_arena, alignment,
        bytes + 1) :
        NULL;
  (void)mutex_unlock(&main_arena.mutex);
...
/* The following hooks are used when the global initialization in
   aproxy_ptmalloc_init() hasn't completed yet. */
...
#endif
{
  ...
  victim = aproxy_int_malloc(&main_arena, sz);
...
#endif
{
  ...
  victim = aproxy_int_memalign(&main_arena, align, sz);
