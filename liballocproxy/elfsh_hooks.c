...
/* A flag that is set by elfsh_malloc_set_state, to signal that malloc checking
   must not be enabled on the request from the user (via the MALLOC_CHECK_
   environment variable).  It is reset by __elfsh_malloc_check_init to tell
   elfsh_malloc_set_state that the user has requested malloc checking.
   ...
   user has requested it elfsh_malloc_set_state just calls __elfsh_malloc_check_init
...
__elfsh_malloc_check_init()
{
  ...
  __elfsh_malloc_hook = malloc_check;
  __elfsh_free_hook = free_check;
  __elfsh_realloc_hook = realloc_check;
  __elfsh_memalign_hook = memalign_check;
...
#else
mem2mem_check(ptr, sz) Void_t *ptr;
size_t sz;
#endif
{
  mchunkptr p;
...
#else
malloc_check(sz, caller) size_t sz;
const Void_t *caller;
#endif
{
  Void_t *victim;
  ...
  victim = (top_check() >= 0) ? elfsh_int_malloc(&main_arena, sz + 1) : NULL;
...
#else
free_check(mem, caller) Void_t *mem;
const Void_t *caller;
#endif
{
  mchunkptr p;
...
#endif
{
  ...
  if (chunk_is_mmapped(oldp))
...
#endif
        {
          ...
          if (oldsize - SIZE_SZ >= nb)
          ...
          else
            {
              ...
              if (top_check() >= 0)
                {
                  newmem = elfsh_int_malloc(&main_arena, bytes + 1);
                }
  ...
  else
    {
      ...
      if (top_check() >= 0)
        {
          newmem = elfsh_int_realloc(&main_arena, oldmem, bytes + 1);
        }
...
#endif
{
  ...
  checked_request2size(bytes + 1, nb);
  (void)mutex_lock(&main_arena.mutex);
  mem = (top_check() >= 0) ? elfsh_int_memalign(&main_arena, alignment,
        bytes + 1) :
        NULL;
  (void)mutex_unlock(&main_arena.mutex);
...
/* The following hooks are used when the global initialization in
   elfsh_ptmalloc_init() hasn't completed yet. */

static Void_t *
#if __STD_C
malloc_starter(size_t sz, const Void_t *caller)
#else
malloc_starter(sz, caller) size_t sz;
const Void_t *caller;
#endif
{
  Void_t *victim;

  victim = elfsh_int_malloc(&main_arena, sz);
...
#else
memalign_starter(align, sz, caller) size_t align, sz;
const Void_t *caller;
#endif
{
  Void_t *victim;

  victim = elfsh_int_memalign(&main_arena, align, sz);
...
#else
free_starter(mem, caller) Void_t *mem;
const Void_t *caller;
#endif
{
  mchunkptr p;
...
/* Get/set state: elfsh_malloc_get_state() records the current state of all
   malloc variables (_except_ for the actual heap contents and `hook'
   function pointers) in a system dependent, opaque data structure.
   This data structure is dynamically allocated and can be free()d
   after use.  elfsh_malloc_set_state() restores the state of all malloc
   ...
   elfsh_malloc_get_state() was called, then elfsh_malloc_set_state() calls
   __elfsh_malloc_check_init() if possible; if malloc checking was not in
  ...
  if (ms->version >= 1)
    {
      ...
      if (ms->using_malloc_checking && !using_malloc_checking &&
        ...
        {
          __elfsh_malloc_hook = 0;
          __elfsh_free_hook = 0;
          __elfsh_realloc_hook = 0;
          __elfsh_memalign_hook = 0;
