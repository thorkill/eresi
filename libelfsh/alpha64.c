      ...
      if (cur[idx] == addr)
        {
          ...
          PROFILER_ROUT(__FILE__, __FUNCTION__, __LINE__,
                        (modgot->shdr->sh_addr + idx * sizeof(eresi_Addr)));
        }
    }
...
elfshsect_t *elfsh_modgot_alpha64(elfshsect_t *infile, elfshsect_t *modrel)
{
  ...
  XALLOC(__FILE__, __FUNCTION__, __LINE__, buff,
         strlen(modrel->parent->name) + 4 + 1, NULL);
  ...
  */
  XALLOC(__FILE__, __FUNCTION__, __LINE__, data, needed * sizeof(eresi_Addr),
         NULL);
          ...
          if (elfsh_get_reltype(relcur) == R_ALPHA_LITERAL)
            {
              ...
              if (!sym || !name)
                PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
                             "Symbol or Name not found for LITERAL",
                             NULL);
