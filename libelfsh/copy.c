...
elfshobj_t  *elfsh_copy_obj(elfshobj_t *file)
{
  ...
  XALLOC(__FILE__, __FUNCTION__, __LINE__, copy->pht,
         sizeof(elfsh_Phdr) * file->hdr->e_phnum, NULL);
  memcpy(copy->pht, file->pht, sizeof(elfsh_Phdr) * file->hdr->e_phnum);
  XALLOC(__FILE__, __FUNCTION__, __LINE__, copy->sht,
         sizeof(elfsh_Shdr) * file->hdr->e_shnum, NULL);
