...
int		elfsh_load_hdr(elfshobj_t *file)
{
  ...
  XALLOC(data, sizeof(elfsh_Ehdr), -1);
  file->hdr = (elfsh_Ehdr *) data;
  if ((len = read(file->fd, file->hdr, sizeof(elfsh_Ehdr))) <= 0)
    ELFSH_PROFILE_ERR(__FILE__, __FUNCTION__, __LINE__, (char *)ELFSH_ERR_ARRAY, 
		      len);
...
int elfsh_get_core_notes(elfshobj_t *file)
{
  ...
  int         i, offset, filesz, j, namesz, descsz;
  int         pos;
  char        name[128];
  ...
  for (i = 0; i != file->hdr->e_phnum; i++) 
      ...
      if (elfsh_get_segment_type(&file->pht[i]) == PT_NOTE) 
	{
	  ...
	  offset = file->core.offset = elfsh_get_segment_offset(&file->pht[i]);
	  filesz = file->core.length = elfsh_get_segment_filesz(&file->pht[i]);
