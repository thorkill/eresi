...
#if __DEBUG_COPYPLT__
      printf("[DEBUG_COPYPLT] Cannot find symbol at PLT OFFSET + %u (.plt + off = "
             XFMT " ) object %s \n",
...
#if __DEBUG_COPYPLT__
  printf("[DEBUG_COPYPLT] Found symbol [%s] for PLT OFFSET + %u (.plt + off = "
         XFMT " ) object %s\n",
         elfsh_get_dynsymbol_name(file, sym), off, (eresi_Addr) plt->shdr->sh_addr + off,
         plt->parent->name);
#endif
