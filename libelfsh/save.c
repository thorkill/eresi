...
static int    elfsh_save_sht(elfshobj_t *file, int fd)
{
  ...
  /* Dump new SHT */
  for (wrindex = index = 0; index < file->hdr->e_shnum;
       index++, sect = sect->next)
    if (!(sect->flags & ELFSH_SECTION_REMOVED))
      {
        memcpy(newsht + wrindex, file->sht + index, file->hdr->e_shentsize);
        newsht[wrindex].sh_link -= elfsh_find_previous_rmnbr(file,
                                   newsht[wrindex].sh_link);
#if __DEBUG_MAP__
...
elfshobj_t  *elfsh_save_preconds(elfshobj_t *file)
{
  ...
  if (actual && strstr(actual->name, ".elfsh"))
    {
      totsize = actual->shdr->sh_addr - sizeof(elfsh_Ehdr) - (sizeof(
                  elfsh_Phdr) * file->hdr->e_phnum);
