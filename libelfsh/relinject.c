    ...
    case STT_NOTYPE:
      if (!elfsh_static_file(enew->parent))
      ...
      else
        {
          ...
          if (dep != NULL && dep != reltab->parent
              && elfsh_inject_etrel(enew->parent, dep) < 0)
            {
              PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
                           "Unable to inject ET_REL dependence", -1);
            }
...
#if __DEBUG_RELADD__
      fprintf(stderr,
              "[DEBUG_RELADD] relocation loop stage %u for section %s index %u \n",
                                       ...
                                       u_int mod)
{
  ...
  XALLOC(__FILE__, __FUNCTION__, __LINE__, newname,
         strlen(sect->parent->name) + strlen(sect->name) + 2, -1);
