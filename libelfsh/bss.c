                           ...
                           char fixflag)
{
  ...
  */
  if (bss->prev != NULL &&
    ...
    {
      size = bss->shdr->sh_offset - (bss->prev->shdr->sh_offset +
                                     bss->prev->shdr->sh_size);
  ...
  */
  ...
  if (bss->data == NULL)
    {
      XALLOC(__FILE__, __FUNCTION__, __LINE__, bss->data, bss->shdr->sh_size + size2,
             -1);
  ...
  /* Extend physical bss and the segment for .bss if PHT exists */
    ...
    {
      XREALLOC(__FILE__, __FUNCTION__, __LINE__, bss->data, bss->data,
               bss->shdr->sh_size + size2, -1);
      bss->phdr->p_filesz += bss->shdr->sh_size + size2;
      bss->phdr->p_memsz  += size2;
...
elfshsect_t *elfsh_insert_bss(elfshobj_t *file, elfshobj_t *rel, char *bssname)
{
  ...
  newbss = elfsh_insert_section(file, buf, NULL, ELFSH_DATA_INJECTION, pagesize,
                                0);
