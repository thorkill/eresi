                        ...
                        elfshsect_t *relplt)
{
  ...
  memcpy(elfsh_readmem(altgot) + altgot->curend, (char *) &gotent,
         sizeof(gotent));
                         ...
                         eresi_Addr addr)
{
  ...
  *(uint32_t *) ((char *) hook + 1) = addr - (hooks->shdr->sh_addr + (hook + 1 -
                                      (char *) elfsh_readmem(hooks)) + 4);
  memcpy(hook + 5, buff, ret);
  memcpy(hook + 5 + ret, "\xe9\x00\x00\x00\x00", 5);
  *(uint32_t *) ((char *) hook + 6 + ret) = symbol->st_value -
      (hooks->shdr->sh_addr + hooks->curend + 10);
  ...
  *(uint32_t *) ((char *) hookbuf + 1) = (hooks->shdr->sh_addr + hooks->curend)
                                         - (symbol->st_value + 5);
 ...
 * @return
 */
int             *elfsh_args_count_ia32(elfshobj_t *file, u_int foffset,
                                       eresi_Addr vaddr)
{
  int           index;
  int           ret;
  ...
  for (icount = index = 0; index < asm_len && data; index += ret, icount++)
    {
      /* Read an instruction */
      if ((ret = asm_read_instr(&i, (u_char *) (data + index), asm_len -  index,
                                &proc)) > 0)
        {
