                           ...
                           eresi_Addr  addr)
{
  ...
  *((uint32_t *) ((char *) (data + hooks->curend) + 0x10)) |= ((
        symbol->st_value + 0x8) & (~ 0xe0000000 )) >> 2;
                                 ...
                                 eresi_Addr addr)
{
  ...
  */
  ...
  XALLOC(__FILE__, __FUNCTION__, __LINE__, opcode,
         (9 + (varnbr * 2)) * sizeof(uint32_t), -1);
  ...
  *originstr = (*originstr & 0xFFFF0000) | diff;
  ...
  for (index = varindex = 0, gotindex = gotno; varindex < varnbr;
       index++, gotindex++)
    if (elfsh_get_symbol_type(sym + index) == STT_OBJECT && !sym[index].st_value)
      {
  ...
  elfsh_writememf(file, altgotprolog->shdr->sh_offset, opcode,
                  sizeof(uint32_t) * 7);
  elfsh_writememf(file, padgot->shdr->sh_offset, opcode + 7,
                  sizeof(uint32_t) * (2 + (varnbr * 2)));
  PROFILER_ROUT(__FILE__, __FUNCTION__, __LINE__, 0);
}
