 ...
 */
int     elfsh_extplt_expand_versym(elfshobj_t *file, elfshsect_t *versym,
                                   char *name)
{
...
int   elfsh_extplt_mirror_sections(elfshobj_t *file)
{
  ...
  name = (FILE_IS_SPARC(file) ? ELFSH_SECTION_NAME_RELAPLT :
          ELFSH_SECTION_NAME_RELPLT);
  ...
  if (relgot)
    {
      ...
      dynent = elfsh_get_dynamic_entry_by_type(file,
               IS_REL(dynamic) ? DT_REL : DT_RELA);
      ...
      enew->shdr->sh_entsize = IS_REL(dynamic) ? sizeof(elfsh_Rel) : sizeof(
                                 elfsh_Rela);
    }
...
elfsh_Sym *elfsh_request_pltent(elfshobj_t *file, char *name)
{
  ...
  sym = elfsh_create_symbol(extplt->shdr->sh_addr + extplt->curend -
