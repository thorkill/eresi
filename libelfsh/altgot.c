        ...
        if (dw == 0x3c1c0000 && dw2 == 0x279c0000)
          {
            ...
            printf("[DEBUG_SHIFTMIPS] Found dw/dw2 at address " XFMT " / " XFMT
                   " (%s) -> %s \n",
                   (eresi_Addr) current->shdr->sh_addr + ((char *) dword  -
                       (char *) current->data),
                   (eresi_Addr) current->shdr->sh_addr + ((char *) dword2 -
                       (char *) current->data),
                   current->name, str);
...
#endif
      PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__, "Unable to get relocation entry",
                   -1);
    }
  ...
  for (index = 0; index < relplt->shdr->sh_size / sizeof(elfsh_Rela); index++)
    {
      ...
      elfsh_set_reloffset((elfsh_Rel *) l,
                          elfsh_get_reloffset((elfsh_Rel *) l) + diff);
    }
                              ...
                              elfshsect_t *altplt)
{
  ...
  /* On SPARC we change it from .plt to .alt.plt : ALTPLT technique */
  else if (FILE_IS_SPARC(file))
    {
      ...
      elfsh_shift_sparc_relocs(file, altplt->shdr->sh_addr - plt->shdr->sh_addr,
                               relplt);
    }
