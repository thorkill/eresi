  ...
  if (sect->curend + len > sect->shdr->sh_size)
    {
      ...
      if (sect->data != NULL)
        {
          XREALLOC(__FILE__, __FUNCTION__, __LINE__, sect->data, sect->data,
                   sect->shdr->sh_size + len, -1);
        }
      else
        {
...
int     elfsh_remove_section(elfshobj_t *obj, char *name)
{
  ...
  obj->hdr->e_shnum--;
  XALLOC(__FILE__, __FUNCTION__, __LINE__, enew,
         obj->hdr->e_shnum * sizeof(elfsh_Shdr), -1);
                               ...
                               u_int      sizelem)
{
  ...
  if (iotype == ELFSH_IOTYPE_STATIC || iotype == ELFSH_IOTYPE_EMBEDDED)
    ...
    {
      elfsh_writemem(sect->parent, sect->shdr->sh_addr + (off * sizelem), data,
                     size);
    }
