...
void
egdb_add_thread (unsigned long thread_id, void *target_data, unsigned int gdb_id)
{
  ...
  set_egdb_inferior_regcache_data (new_thread, egdb_new_register_cache ());
...
{
  struct thread_info *thread = get_thread (inf);
  egdb_free_register_cache (egdb_inferior_regcache_data (thread));
  free (thread);
}

void
egdb_remove_thread (struct thread_info *thread)
{
...
void *
egdb_inferior_target_data (struct thread_info *inferior)
{
  return inferior->target_data;
}

void
set_egdb_inferior_target_data (struct thread_info *inferior, void *data)
{
  inferior->target_data = data;
}

void *
egdb_inferior_regcache_data (struct thread_info *inferior)
{
  return inferior->regcache_data;
}

void
set_egdb_inferior_regcache_data (struct thread_info *inferior, void *data)
{
...
void
egdb_loaded_dll (const char *name, CORE_ADDR base_addr)
{
...
void
unegdb_loaded_dll (const char *name, CORE_ADDR base_addr)
{
  struct dll_info *dll;
...
void
egdb_add_pid_to_list (struct inferior_list *list, unsigned long pid)
{
...
int
egdb_pull_pid_from_list (struct inferior_list *list, unsigned long pid)
{
  struct inferior_list_entry *new_entry;
