...
/* This table must match in order and size the signals in enum egdb_target_signal
...
char *
egdb_target_signal_to_string (enum egdb_target_signal sig)
{
...
char *
egdb_target_signal_to_name (enum egdb_target_signal sig)
{
  if ((int) sig >= TARGET_SIGNAL_FIRST && (int) sig <= TARGET_SIGNAL_LAST
...
enum egdb_target_signal
egdb_target_signal_from_name (char *name)
{
  enum egdb_target_signal sig;
  ...
  for (sig = TARGET_SIGNAL_HUP;
       ...
       sig = (enum egdb_target_signal) ((int) sig + 1))
...
enum egdb_target_signal
egdb_target_signal_from_host (int hostsig)
{
  ...
  if (hostsig >= REALTIME_LO && hostsig < REALTIME_HI)
	...
	return (enum egdb_target_signal)
	...
	error ("GDB bug: target.c (egdb_target_signal_from_host): unrecognized real-time signal");
    }
#endif
...
/* Convert a OURSIG (an enum egdb_target_signal) to the form used by the
   target operating system (refered to as the ``host'') or zero if the
   equivalent host signal is not available.  Set/clear OURSIG_OK
   accordingly. */

static int
do_egdb_target_signal_to_host (enum egdb_target_signal oursig,
			  int *oursig_ok)
{
  int retsig;
...
int
target_signal_to_host_p (enum egdb_target_signal oursig)
{
  int oursig_ok;
  do_egdb_target_signal_to_host (oursig, &oursig_ok);
  return oursig_ok;
}

int
egdb_target_signal_to_host (enum egdb_target_signal oursig)
{
  int oursig_ok;
  int targ_signo = do_egdb_target_signal_to_host (oursig, &oursig_ok);
  if (!oursig_ok)
    {
      /* The user might be trying to do "signal SIGSAK" where this system
         doesn't have SIGSAK.  */
      warning ("Signal %s does not exist on this system.\n",
	       egdb_target_signal_to_name (oursig));
      return 0;
    }
  else
    return targ_signo;
...
enum egdb_target_signal
egdb_target_signal_from_command (int num)
{
  if (num >= 1 && num <= 15)
    return (enum egdb_target_signal) num;
