...
i386_fill_gregset (void *buf)
{
  ...
  for (i = 0; i < i386_num_regs; i++)
    egdb_collect_register (i, ((char *) buf) + i386_regmap[i]);

  egdb_collect_register_by_name ("orig_eax", ((char *) buf) + ORIG_EAX * 4);
}
...
i386_store_gregset (const void *buf)
{
  ...
  for (i = 0; i < i386_num_regs; i++)
    egdb_supply_register (i, ((char *) buf) + i386_regmap[i]);

  egdb_supply_register_by_name ("orig_eax", ((char *) buf) + ORIG_EAX * 4);
}

static void
i386_fill_fpregset (void *buf)
{
  egdb_i387_cache_to_fsave (buf);
}

static void
i386_store_fpregset (const void *buf)
{
  egdb_i387_fsave_to_cache (buf);
}

static void
i386_fill_fpxregset (void *buf)
{
  egdb_i387_cache_to_fxsave (buf);
}

static void
i386_store_fpxregset (const void *buf)
{
  egdb_i387_fxsave_to_cache (buf);
}
...
i386_get_pc ()
{
  ...
  egdb_collect_register_by_name ("eip", &pc);
...
i386_set_pc (CORE_ADDR newpc)
{
  ...
  egdb_supply_register_by_name ("eip", &newpc);
}
...
i386_breakpoint_at (CORE_ADDR pc)
{
  ...
  egdb_read_inferior_memory (pc, &c, 1);
  if (c == 0xCC)
    return 1;
