...
egdb_remote_open (char *name)
{
  ...
  if (port_str == NULL)
      ...
      else
	{
      ...
      if (remote_desc < 0)
	egdb_perror_with_name ("Could not open remote device");
      ...
      if (!winsock_initialized)
	{
      ...
      if (tmp_desc < 0)
	egdb_perror_with_name ("Can't open socket");
	  ...
	  || listen (tmp_desc, 1))
	egdb_perror_with_name ("Can't bind address");
      ...
      if (port == 0)
	{
	  ...
	  if (getsockname (tmp_desc, (struct sockaddr *) &sockaddr, &len) < 0
	      || len < sizeof (sockaddr))
	    egdb_perror_with_name ("Can't determine port");
      ...
      if (remote_desc == -1)
	egdb_perror_with_name ("Accept failed");
...
int
egdb_unegdb_hexify (char *bin, const char *hex, int count)
{
  int i;
...
void
egdb_decode_address (CORE_ADDR *addrp, const char *start, int len)
{
  CORE_ADDR addr;
  char ch;
  int i;
...
const char *
egdb_decode_address_to_semicolon (CORE_ADDR *addrp, const char *start)
{
  const char *end;
  ...
  egdb_decode_address (addrp, start, end - start);
...
int
egdb_hexify (char *hex, const char *bin, int count)
{
  int i;
...
int
egdb_putpkt_binary (char *buf, int cnt)
{
  int i;
  ...
  *p = '\0';
  ...
  do
      ...
      if (write (remote_desc, buf2, p - buf2) != p - buf2)
	{
      ...
      if (remote_debug)
	{
	  fprintf (stderr, "egdb_putpkt (\"%s\"); [looking for ack]\n", buf2);
	  fflush (stderr);
	}
	...
	{
	  if (cc == 0)
	    fprintf (stderr, "egdb_putpkt(read): Got EOF\n");
	  else
	    perror ("egdb_putpkt(read)");
...
int
egdb_putpkt (char *buf)
{
  return egdb_putpkt_binary (buf, strlen (buf));
}
...
int		egdb_getpkt (char *buf)
{
  ...
  while (1)
      ...
      while (1)
	{
	  ...
	  if (remote_debug)
	    {
	      fprintf (stderr, "[egdb_getpkt: discarding char '%c']\n", c);
	      fflush (stderr);
	    }
  ...
  if (remote_debug)
    {
      fprintf (stderr, "egdb_getpkt (\"%s\");  [sending ack] \n", buf);
      fflush (stderr);
    }
...
outreg (int regno, char *buf)
{
  ...
  *buf++ = tohex (regno & 0xf);
  *buf++ = ':';
  egdb_collect_register_as_string (regno, buf);
  buf += 2 * egdb_register_size (regno);
  *buf++ = ';';
...
void
egdb_dead_thread_notify (int id)
{
  char own_buf[256];
...
void
egdb_prepare_resume_reply (char *buf, char status, unsigned char sig)
{
  int nib;
  ...
  if (status == 'T')
	  ...
	  && (*the_target->stopped_by_watchpoint) ())
	{
	  ...
	  *buf++ = ';';
      ...
      while (*regp)
	{
	  buf = outreg (egdb_find_regno (*regp), buf);
	  regp ++;
	}
...
int
egdb_look_up_one_symbol (const char *name, CORE_ADDR *addrp)
{
  ...
  egdb_hexify (own_buf + strlen ("qSymbol:"), name, strlen (name));
  if (egdb_putpkt (own_buf) < 0)
    return -1;

  /* FIXME:  Eventually add buffer overflow checking (to egdb_getpkt?)  */
  len = egdb_getpkt (own_buf);
  if (len < 0)
    return -1;
  ...
  while (own_buf[0] == 'm')
    {
      ...
      if (egdb_read_inferior_memory (mem_addr, mem_buf, mem_len) == 0)
  ...
  egdb_decode_address (addrp, p, q - p);
...
void
egdb_monitor_output (const char *msg)
{
  char *buf = malloc (strlen (msg) * 2 + 2);

  buf[0] = 'O';
  egdb_hexify (buf + 1, msg, 0);
...
char *
egdb_xml_escape_text (const char *text)
{
  char *result;
  int i, special;
