...
void    ke2dbg_commands_install()
{
  ...
  revm_command_add(CMD_INTHOOK, (void *) cmd_rr0d_inthook,
                   (void *) revm_getoption, 0, "");
...
int   cmd_rr0d_bpm()
{
  ke2dbg_hwbp_set(ke2dbg_virt2lin_convert(current_reg_stack.reg_CS,
                                          adresse_ret_1));
  return (0);
}
...
int   cmd_rr0d_inthook()
{
  ...
  if (is_int_hooked[adresse_ret_1])
    tmp_int[adresse_ret_1] = (void *) ke2dbg_idt_modify( (unsigned int)
                             old_int[adresse_ret_1],
  ...
  else
    {
      ...
      old_int[adresse_ret_1] = (void *) ke2dbg_idt_modify( (unsigned int)
                               addrnewint[adresse_ret_1],
                               (unsigned int *)idt_[0] + 2 * adresse_ret_1);
    }
