...
#if 0
      printf("%.8X %.8X %.3X %.8X ", ke2dbg_pageinfo_print, adr, info,
             i * 0x00400000);
 ...
 * @return
 */
unsigned int ke2dbg_pageinfo_set(unsigned int adresse, unsigned int data,
                                 unsigned int level)
{
  unsigned int *page_tab;
  unsigned int *sub_page_tab;
...
unsigned int kedbg_physpage_get(unsigned int ad)
{
  ...
  for (i = 0; i < 0xFFFF0000; i += 0x1000)
    {
      info_page_src = ke2dbg_pageinfo_get((unsigned int)i, NULL, NULL,
                                          &level ) & 0xFFFFF000;
...
void poked1(unsigned int ptr, unsigned int v)
{
  unsigned int info_page_src;
  unsigned int level;
  unsigned char *ptr_flying = (unsigned char *)(((unsigned int)
                              &flying_page1[0x1000]) & 0xFFFFF000);
  ...
  info_page_src = ke2dbg_pageinfo_get((unsigned int)ptr_flying, NULL, NULL,
                                      &level );
  ...
  ke2dbg_pageinfo_set((unsigned int)ptr_flying,
                      ptr_hi + (info_page_src & 0x00000EFF), level);
