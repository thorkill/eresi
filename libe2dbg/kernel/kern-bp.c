...
int ke2dbg_is_breakpoint(unsigned char *ptr)
{
  ...
  for (i = 0; i < NUM_BREAKPOINTS; i++)
    if (list_breakpoints[i].address == (unsigned long) ptr)
      {
        break;
      }
  ...
  for (i = 0; i < NUM_BREAKPOINTS; i++)
    {
      ...
      if (list_breakpoints[i].paddress == padresse)
        {
          break;
        }
    }
  ...
  for (i = 0; i < 4; i++)
    {
      if (list_hw_breakpoints[i].address)
        {
          ke2dbg_sprintf(debug__, "%.4X %.8X", i + HW_MAGIC_VALUE,
                         list_hw_breakpoints[i].address);
          ke2dbg_output(debug__);
        }
    }
}
