  ...
  do
    {
      ...
      if (!(mychar & IBUFFER_FULL))
        {
          break;  // if the flag is clear, we go ahead
        }
    }
  while (i--);
...
long    back_disasm(unsigned int cur_eip, int min_back_line)
{
  ...
  while (num_instr > min_back_line)
    {
      testing_eip += asm_read_instr(&my_ins, (u_char *) testing_eip, 32,
                                    world.curjob->proc);
      num_instr--;
    }
...
void ke2dbg_cmdkey_insert(char
                          car) /* A new key is pressed and is added to the command */
{
