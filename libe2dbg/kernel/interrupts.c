...
void ke2dbg_register_print(Reg_struct *regstack)
{
  ...
  screen_colors[2][58] = (regstack->reg_EFLAG & 0x400) ==
                         (old_reg_data.reg_EFLAG & 0x400) ? ' ' : '1';
  screen_colors[2][59] = (regstack->reg_EFLAG & 0x200) ==
                         (old_reg_data.reg_EFLAG & 0x200) ? ' ' : '1';
  screen_colors[2][60] = (regstack->reg_EFLAG & 0x100) ==
                         (old_reg_data.reg_EFLAG & 0x100) ? ' ' : '1';
  screen_colors[2][61] = (regstack->reg_EFLAG & 0x80) == (old_reg_data.reg_EFLAG &
                         0x80) ? ' ' : '1';
  screen_colors[2][62] = (regstack->reg_EFLAG & 0x40) == (old_reg_data.reg_EFLAG &
                         0x40) ? ' ' : '1';
  screen_colors[2][63] = (regstack->reg_EFLAG & 0x10) == (old_reg_data.reg_EFLAG &
                         0x10) ? ' ' : '1';
  screen_colors[2][64] = (regstack->reg_EFLAG & 0x4)  == (old_reg_data.reg_EFLAG &
                         0x4) ? ' ' : '1';
  screen_colors[2][65] = (regstack->reg_EFLAG & 0x1)  == (old_reg_data.reg_EFLAG &
                         0x1) ? ' ' : '1';
...
void ke2dbg_printf_disasm(eresi_Addr selector, eresi_Addr current_eip)
{
  ...
  if ( (!ke2dbg_page_exist(virtual_eip))
       || (!ke2dbg_page_exist(virtual_eip + 0x70)) )
    {
      for (i = 0; i < 0x70; i++)
        {
          tab_true_byte[i] = '?';
  ...
  for (i = 0; i + decal_printf < code_win_dim; i++, asm_lines_adresse_n++)
    {
      ...
      symbol_name = elfsh_reverse_symbol(world.curjob->curfile, current_eip + x,
                                         NULL);
      ...
      /*end symbol*/
      index_chaine += ke2dbg_sprintf(asm_line, "%.4X:%.8X ", selector,
                                     current_eip + x);
      ...
      for (ii = 0 ; ii < vic; ii++)
        {
          index_chaine += ke2dbg_sprintf(asm_line + index_chaine, "%.2X",
                                         tab_true_byte[x + ii] & 0xff);
        }
      ...
      if ((current_eip + x) == ke2dbg_virt2lin_convert(selector,
          (unsigned int)reg_stack->reg_EIP))
        {
          asm_line[77] = '<';
        }
...
void ke2dbg_step (void)
{
  ...
  if (fired)
    {
      adresse = (unsigned char *) ke2dbg_virt2lin_convert(reg_stack->reg_CS,
                reg_stack->reg_EIP);
      ...
      if (fired)
        {
          ...
          for (i = 0; i < asm_lines_adresse_n; i++)
            if (ke2dbg_virt2lin_convert(reg_stack->reg_CS,
                                        (unsigned int)reg_stack->reg_EIP) ==
                asm_lines_adresse[i])
              {
                eip_in_asm_win = 1;
  ...
  if ( ((key_scan_code & 0xFF) == STEP_INTO)
       || (((key_scan_code & 0xFF) == STEP_OVER) && (!ret)))
    {
      step_on = 1;
  ...
  if (((key_scan_code & 0xFF) == STEP_OVER) && (ret))
    {
      ...
      step_on = 0;
      reg_stack->reg_EFLAG = 0xfffffeff & reg_stack->reg_EFLAG;
      ke2dbg_insert_bp(current_reg_stack.reg_CS, current_reg_stack.reg_EIP + ret,
                       BP_ONE_SHOT);
    }
...
void  _ke2dbg_int3_handle (void)
{
  ...
  */
  ...
  if ( (bp_num = ke2dbg_is_breakpoint((unsigned char *)reg_stack->reg_EIP - 1)) !=
       -1)
    {
      reg_stack->reg_EIP--;
      ...
      if ((bp_num == unmapped_bp_to_shoot) && (unmapped_bp_to_shoot != -1))
        {
          ke2dbg_sprintf(debug_, "ke2dbg is deleting a unwanted bp@ %.8X n %d",
                         reg_stack->reg_EIP, bp_num);
          ke2dbg_logbuf_insert( debug_);
...
int ke2dbg_init(void)
{
  ...
  oldkbd = (void *) ke2dbg_idt_modify(addrnewkbd,
                                      (unsigned int *)idt + 2 * KEYB_INTER);
#endif /* DONT_HOOK_KBD */
  ...
  for (i = 0; i < MAX_INT; i++)
    {
      if (is_int_hooked[i])
        {
          old_int[i] = (void *) ke2dbg_idt_modify(addrnewint[i],
                                                  (unsigned int *)idt + 2 * i);
        }
    }
...
void ke2dbg_cleanup(void)
{
  ...
  ke2dbg_hwbp_reset();
#ifndef DONT_HOOK_KBD
  tmpkbd = (void *) ke2dbg_idt_modify( (unsigned int)oldkbd,
                                       (unsigned int *)idt_[0] + 2 * KEYB_INTER);
#endif /* DONT_HOOK_KBD */
  ...
  for (i = 0; i < MAX_INT; i++)
    {
      if (is_int_hooked[i])
        {
          tmp_int[i] = (void *) ke2dbg_idt_modify( (unsigned int)old_int[i],
                       (unsigned int *)idt_[0] + 2 * i);
        }
    }
