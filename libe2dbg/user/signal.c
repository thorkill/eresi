...
void    e2dbg_watch_check_ia32_sysv(u_int regidx, char *regstr)
{
  ...
  for (idx = 0; idx < E2DBG_STEPCMD_MAX && e2dbgworld.tracedata[idx]; idx++)
    if (e2dbgworld.curthread->context->uc_mcontext.gregs[regidx] ==
        e2dbgworld.tracedata[idx])
      {
 ...
 * @param
 */
void            e2dbg_sigsegv_handler(int signum, siginfo_t *info,
                                      void *pcontext)
{
  char    *argv[2];
  //ucontext_t  *context;
 ...
 * @return
 */
void            e2dbg_sigint_handler(int signum, siginfo_t *info,
                                     void *pcontext)
{
  char    *argv[2];
  e2dbgparams_t params;
 ...
 * @return
 */
void            e2dbg_sigstop_handler(int signum, siginfo_t *info,
                                      void *pcontext)
{
  char    *argv[2];
  e2dbgparams_t params;
 ...
 * @return
 */
void            e2dbg_thread_sigusr2(int signum, siginfo_t *info,
                                     void *pcontext)
{
  e2dbgthread_t *curthread;
  char    key[15];
...
#if __DEBUG_THREADS__
  fprintf(stderr,
          " ********** [T] Thread %u getting out of SIGUSR2 handler ******* \n",
          (unsigned int) curthread->tid);
#endif
 ...
 * @return
 */
void            e2dbg_sigtrap_handler(int signum, siginfo_t *info,
                                      void *pcontext)
{
  char    *argv[2];
  //ucontext_t  *context;
...
void      e2dbg_breakpoint_reinstall()
{
  ...
  if (bp)
    ...
    fprintf(stderr, " [D] Breakpoint was deleted from " AFMT
            " : not reinstalling ! \n",
...
#if __DEBUG_BP__
  fprintf(stderr,
          " [D] At PC = %08X : Trying to find breakpoint at addr %08X (bpsize = %u)\n",
          *pc, *pc - bpsz, bpsz);
#endif
...
#if __DEBUG_BP__
      fprintf(stderr,
              " [D] At PC = 0x%X : returned from BP handler with step enabled\n",
              *pc);
#endif
    }
}
                                   ...
                                   void   *pcontext)
{
  ...
  if (!e2dbgworld.stoppedthread->count)
    ...
    fprintf(stderr,
            " NOT COLLECTING THREADS CONTEXTS (stopped thread %u state count = %u) \n",
  ...
  if (e2dbgworld.stoppedthread->count == E2DBG_BREAK_FINISHED ||
    ...
    fprintf(stderr,
            " [D] No SIGCONT as thread (%u) is stopped with state count = %u \n",
            (u_int) e2dbgworld.stoppedthread->tid, e2dbgworld.stoppedthread->count);
