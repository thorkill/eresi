...
static void    *e2dbg_thread_start(void *param)
{
  ...
  ret = pthread_attr_getstack(&attr, (void **) &cur->stackaddr,
                              (size_t *) &cur->stacksize);
  ...
  if (ret)
  ...
  else
    {
      ret = snprintf(logbuf, BUFSIZ,
                     " [D] Thread ID %u has stack at addr %08X with size %u \n",
...
#if (__DEBUG_THREADS__ || __DEBUG_BP__)
  fprintf(stderr,
          " [*] Continuing all threads (current number of threads = %u) \n",
          e2dbgworld.threadnbr);
  e2dbg_threads_print();
#endif
