                          ...
                          eresi_Addr  *retaddr)
{
...
#if __DEBUG_FLOW__
      fprintf(stderr,
              " [D] Found contiguous block ! stopping this recursion branch ******** \n");
#endif
  ...
  else if ((curins->type & ASM_TYPE_BRANCH)
           && !(curins->type & ASM_TYPE_CONDCONTROL))
    {
      /* Imperative branch */
      *dstaddr = mjr_get_jmp_destaddr(context);
  ...
  switch (context->proc.type)
      ...
      if ((ins->op[0].type & ASM_OP_VALUE) && !(ins->op[0].type & ASM_OP_REFERENCE))
        {
          ...
          dest = asm_dest_resolve(&context->proc, context->hist[MJR_HISTORY_CUR].vaddr,
                                  (dest + ilen));
        }
    ...
    case ASM_PROC_MIPS:
      switch (ins->instr)
        ...
        case ASM_MIPS_BAL:
          dest = (context->hist[MJR_HISTORY_CUR].vaddr + (((short) ins->op[0].imm + 1) *
                  4));
          break;

        default:
          dest = (context->hist[MJR_HISTORY_CUR].vaddr + (((short) ins->op[1].imm + 1) *
                  4));
          break;
        }
    ...
    case ASM_PROC_ARM:
      if (!(ins->type & ASM_TYPE_INDCONTROL))
        /* BLX(1) & BL */
        dest = asm_dest_resolve_arm(context->hist[MJR_HISTORY_CUR].vaddr,
                                    ins->op[0].imm,
  ...
  switch (context->proc.type)
    ...
    case ASM_PROC_MIPS:
      ...
      if (ins->op[i].type & ASM_MIPS_OTYPE_BRANCH)
        {
          dest = (context->hist[MJR_HISTORY_CUR].vaddr + (((short)ins->op[i].imm + 1) *
                  4));
        }
