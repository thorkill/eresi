...
int     mjr_blocks_get(mjrcontext_t *ctxt)
{
  ...
  sect = elfsh_get_section_by_name(ctxt->obj, ELFSH_SECTION_NAME_EDFMT_BLOCKS, 0,
                                   0, 0);
...
int   mjr_block_dump(mjrcontext_t *ctxt, container_t *c)
{
  ...
  fprintf(D_DESC, "[D] %s: vaddr:" AFMT
          " size:%d in container ID:%d OUT:%d IN:%d\n",
          __FUNCTION__, blk->vaddr, blk->size, c->id, c->outlinks->elmnbr,
          c->inlinks->elmnbr);
  ...
  for (llnk = c->inlinks->head; llnk; llnk = llnk->next)
    {
      ...
      fprintf(D_DESC, "[D] %s: " AFMT " linked IN from vaddr:" AFMT
              " size:%d type:%d\n",
  ...
  for (llnk = c->outlinks->head; llnk; llnk = llnk->next)
    {
      lnk = (mjrlink_t *) llnk->data;
      tblk = (mjrblock_t *) mjr_lookup_container(ctxt, lnk->id)->data;
      fprintf(D_DESC, "[D] %s: " AFMT " linked OUT to vaddr:" AFMT
              " size:%d type:%d\n",
              __FUNCTION__, blk->vaddr, tblk->vaddr, tblk->size, lnk->type);
    }
