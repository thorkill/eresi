                                  ...
                                  u_int fieldnbr)
{
  ...
  if (newtype && (newtype->type < ASPECT_TYPE_CORENUM
                  || newtype->type >= ASPECT_TYPE_BASENUM))
    PROFILER_ERR(__FILE__, __FUNCTION__, __LINE__,
  ...
  if (!supertype)
    ...
    {
      newtype = aspect_type_copy_by_name(supertype, label, &fields_hash, 0,
                                         ASPECT_TYPE_MAXPTRDEPTH);
      curoff = newtype->size;
    }
