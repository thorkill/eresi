 ...
 */
static char          *gdbwrap_run_length_decode(char *dstpacket,
    const char *srcpacket,
    unsigned maxsize)
{
...
static Bool          gdbwrap_check_ack(gdbwrap_t *desc)
{
  ...
  else if (desc->packet[0] != GDBWRAP_BAD_CHECKSUM)
    {
      return FALSE;
    }
  else
 ...
 * by the gdbserver.
 */
void                 gdbwrap_setbp(gdbwrap_t *desc, la32 linaddr,
                                   void *datasaved)
{
  u_char             bp = 0xcc;
  char               *ret;
